...
data <- read.csv("C:/University at Buffalo/Semester 2/Optimization methods in machine learning/Project 1/Majority Vote Text.csv")
nrow(data)
ncol(data)
summary(data)
fit_trees <- lm(data$Trees ~ data$Krishna+data$Ravana+data$Lord+data$Ram+data$Sita+data$Tree+data$cows+data$rose)
summary(fit_trees)
...
test.set<-data[-train.rows,]
fit_trainset<-lm(data$Trees ~ data$Krishna+data$Ravana+data$Lord+data$Ram+data$Sita+data$Tree+data$cows+data$rose)
...
ncol(data)
summary(data)
fit_animals <- lm(data$Animal ~ data$Krishna+data$Ravana+data$Lord+data$Ram+data$Sita+data$Tree+data$cows+data$rose)
summary(fit_animals)
...
test.set<-data[-train.rows,]
test.set<-data[-train.rows,]
fit_trainset<-lm(data$Animal ~ data$Krishna+data$Ravana+data$Lord+data$Ram+data$Sita+data$Tree+data$cows+data$rose)
...
ncol(data)
summary(data)
fit_M1 <- lm(data$Mythology_A2_A3_A5 ~ data$Krishna+data$Ravana+data$Lord+data$Ram+data$Sita+data$Tree+data$cows+data$rose)
summary(fit_M1)
...
test.set<-data[-train.rows,]
test.set<-data[-train.rows,]
fit_trainset<-lm(data$Mythology_A2_A3_A5 ~ data$Krishna+data$Ravana+data$Lord+data$Ram+data$Sita+data$Tree+data$cows+data$rose)
...
ncol(data)
summary(data)
fit_M2 <- lm(data$Mythology_A6_A7 ~ data$Krishna+data$Ravana+data$Lord+data$Ram+data$Sita+data$Tree+data$cows+data$rose)
summary(fit_M2)
